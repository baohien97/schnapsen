...
class State:
    ...
    def next(self,
        ...
        if move[1] is not None:
            ...
            state.__deck.add_to_perspective(util.other(state.whose_turn()), move[1],
                                            "P" + str(state.whose_turn()) + "H")
    ...
    def clone(self, signature=None):
        ...
        """
        state = State(self.__deck.clone(signature), self.__player1s_turn, self.__p1_points, self.__p2_points,
                      self.__p1_pending_points, self.__p2_pending_points)
    ...
    @staticmethod
    def generate(id=None, phase=1, project = True):
        """
    ...
    def get_deck(self):
        # return the Deck object associated with the state
        return self.__deck

    def get_card_states(self):
        # return card_state of state
        return self.__deck.get_card_states()

    def get_trump_card_index(self):
        # return index of open trump card in deck
        return self.__deck.get_trump_card_index()

    def set_card(self, index, c_state):
        # sets specified card to specified index
        self.__deck.set_card(index, c_state)

    def get_stock(self, index, card):
        return self.__deck.get_stock(index)

    def get_played_card(self):
        trick = self.__deck.get_trick()
        return trick

    def change_stock(self, index, stock):
        self.__deck.change_stock(index, stock)
        pass

    def change_knowledge(self, player, index, c_state):
        self.__deck.change_knowledge(player, index, c_state)